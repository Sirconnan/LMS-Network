...
class Interface:
    ...
    def new_server(self):
        ...
        ip_lablel = Label(server_gui, text="IP of the server").pack()
        ip_input = Entry(server_gui)
        ip_input.pack()
        ip_input.insert(0, Interface.__ip)

        port_lablel = Label(server_gui, text="Port of the server").pack()
        ...
        def start_server():
            ...
            if askyesno("Checking information", f"Are you sure of connect information:\n- IP: {ip}\n- Port: {port}"):
                ...
                frame_stdout = Frame(server_gui)
                frame_stdout.pack(fill=BOTH, expand=True)

                text_stdout = Text(frame_stdout, wrap="word", height=10, width=50)
                text_stdout.pack(fill=BOTH, expand=True)

                sys.stdout = stdout_redirector(text_stdout) # => remplace the method of sys
...
tmp = Interface(150, 50, "Server App")
